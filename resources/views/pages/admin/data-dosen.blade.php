        ...
        <div class="card-body">
            <div class="table-responsive text-nowrap">
                <table class="table table-hover align-middle" id="tableData">
                    ...
                    <tbody id="tableBody" data-base-number="{{ $dosenList->firstItem() ?? 0 }}">
                        @forelse($dosenList as $index => $dosen)
                            <tr class="searchable-row" data-search="{{ strtolower(($dosen->name ?? '') . ' ' . ($dosen->nidn ?? '') . ' ' . ($dosen->email ?? '') . ' ' . implode(' ', $dosen->roles ?? [])) }}">
                                <td class="row-number">{{ $dosenList->firstItem() + $index }}</td>
                                <td>{{ $dosen->name ?? '-' }}</td>
                                <td>{{ $dosen->nidn ?? '-' }}</td>
                                <td>{{ $dosen->email ?? '-' }}</td>
                                <td>
                                    <form method="POST" action="{{ route('admin.users.updateRole', $dosen->id) }}" class="d-flex gap-2">
                                        @csrf
                                        @method('PATCH')
                                        <select name="role" class="form-select form-select-sm" style="min-width: 140px;">
                                            <option value="dosen" {{ $dosen->hasRole('dosen') ? 'selected' : '' }}>Dosen</option>
                                            <option value="koor" {{ $dosen->hasRole('koor') ? 'selected' : '' }}>Koor</option>
                                        </select>
